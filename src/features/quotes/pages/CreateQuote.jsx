// features/quotes/pages/CreateQuote.jsx
...
const CreateQuote = () => {
  ...
  console.log('🎯 CreateQuote rendered', { 
    loading, 
    error, 
    success, 
  ...
  // Check if we have a pre-selected customer from navigation state
  useEffect(() => {
    console.log('📦 Checking location state for selected customer:', location.state);
    if (location.state?.selectedClient) {
      const { selectedClient } = location.state;
      console.log('✅ Selected customer found:', selectedClient);
      setInitialData({
  ...
  const handleSubmit = async (quoteData) => {
    console.log('🎯 handleSubmit called in CreateQuote with data:', quoteData);
    setSubmitError(null);
  ...
  return (
    <Box>
      <PageHeader
        ...
        subtitle="Fill in the details below to create a new quote for your customer."
        actions={
          <>
            {/* <CustomButton
              label="Save as Draft"
